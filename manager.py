...
def init():
    print('MAKE SURE YOU RUN "manager.py" WITH ADMINISTRATOR RIGHTS!\n')

    global kaliDir, WinDir, OpenVPNClientPath
    if os.path.exists(".env"):
        kaliDir = os.getenv("KALI_DIR")
        WinDir = os.getenv("WIN_DIR")
        OpenVPNClientPath = os.getenv("OPEN_VPN_CLIENT_PATH")
    else:
        print("ERROR: .env file not found and keys are empty!")
        exit()

def runMutationGenerator():
    print("Do you want to run mutation generator? y/n (yes by default):")
    answer = input()
    flag = False
    while (not flag):
        if answer.lower == 'y' or answer == '':
            ...
            print("Running mutation generator in background...")
            generator_proc = subprocess.Popen(["python", "mutation_generato.py"])
        elif answer.lower == 'n':
            flag = True
            print("Continuing without running mutation generator...")
        else:
...
def runHooks():
    ...
    kali_command = f"cd {kaliDir} && echo 'maks' | sudo -S .venv/bin/frida -l 'Stalker.js' -p {pid}"
    full_cmd = f'start "{window_title}" cmd /k wsl -d kali-linux -- bash -c "{kali_command}"'
    subprocess.Popen(full_cmd, shell=True)

    window_title = "hook.js"
    kali_command = f"cd {kaliDir} && echo 'maks' | sudo -S .venv/bin/frida -l 'hook.js' -p {pid}"
    full_cmd = f'start "{window_title}" cmd /k wsl -d kali-linux -- bash -c "{kali_command}"'
    subprocess.Popen(full_cmd, shell=True)

    window_title = "hookControl.js"
    kali_command = f"cd {kaliDir} && echo 'maks' | sudo -S .venv/bin/frida -l 'hookControl.js' -p {pid}"
    full_cmd = f'start "{window_title}" cmd /k wsl -d kali-linux -- bash -c "{kali_command}"'
    subprocess.Popen(full_cmd, shell=True)

        # Ждем запуска хуков
    time.sleep(10.0)
    print("Hooks are placed.")
                        ...
                        try:
                            while True:
                                ...
                                client_proc = subprocess.Popen([
                                    OpenVPNClientPath,
                                        "--config", f"{WinDir}/msi_openvpn_remote_access_l3.ovpn"
                                ])
